...
use crate::mm::page_table::translated_physical_address;
...
pub fn current_translated_physical_address(ptr: *const u8) -> isize {
    translated_physical_address(current_user_token(), ptr) as isize
}
...
pub fn mummap(start: usize, len: usize) -> isize {
    
    let mut inner = TASK_MANAGER.inner.exclusive_access();
    let current = inner.current_task;
    inner.tasks[current].memory_set.mummap(start, len)
}